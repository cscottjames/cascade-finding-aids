<!-- Adapted from EAD Cookbook Style 6, Version 0.9-->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/ead">
        ...
        <div id="ead-contact-info">
          <h4>Contact Information:</h4>
          <p>Mailing Address: The Catholic University of America, Washington, D.C. 20064</p>
          <p>Telephone: 202-319-5065</p>
          <p>Email: <a href="mailto:lib-archives@cua.edu">lib-archives@cua.edu</a></p>
          <p>URL: <a href="https://libraries.catholic.edu/special-collections/archives/">https://libraries.catholic.edu/special-collections/archives/</a></p>
          <hr/>
        </div>
          <xsl:call-template name="toc"/>
          <!--To change the order of display, adjust the sequence of the following apply-template statements which invoke the various templates that populate the finding aid.  Multiple statements are included to handle the possibility that descgrp has been used as a wrapper to replace add and admininfo.  In several cases where multiple elemnents are displayed together in the output, a call-template statement is used-->
...
a matching element with content in the finding aid.-->
		...
		<xsl:if test="string(archdesc/dsc/head)">
			...
			or @level='subgrp' or @level='subcollection']">
				<p>
					<b>
						<a>
							<xsl:attribute name="href"><xsl:text>#series</xsl:text><xsl:number count="c01" from="dsc"/></xsl:attribute>
							<xsl:choose>
	...
	<!-- JORDAN, HERE'S THE HANDLING FOR EXTPTR AS SHOWN IN MY EMAIL -->
	<xsl:template match="extptr">
		<xsl:choose>
			<xsl:when test="@show='embed'">
				<img src="{@href}" altrender="{@title}"/>
			</xsl:when>
...
inserting the contents if there is or adding one if there isn't. -->
	...
	<xsl:template match="archdesc/arrangement/list/item">
		<div>
			<a>
